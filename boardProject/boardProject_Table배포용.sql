...
WHERE COMMENT_DEL_FL = 'N' -- 댓글이 삭제되지 않은 것
OR 0 != (SELECT COUNT(*) FROM "COMMENT" SUB
WHERE SUB.PARENT_COMMENT_NO = C.COMMENT_NO
AND COMMENT_DEL_FL = 'N')
-- 자식 댓글 중 삭제되지 않은 행이 한 개라도 있으면 부모 댓글을 조회 대상에 포함
-- 현재 댓글(C.COMMENT_NO)을 부모로 하는 자식 댓글 중 삭제되지 않은 행의 개수 카운트
-- => 삭제되지 않았거나 삭제되었더라도 그 아래에 활성 상태의 자식 댓글이 존재하면 조회되도록 하는 목적의 SQL
