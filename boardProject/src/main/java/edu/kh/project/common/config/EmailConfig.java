...
public class EmailConfig {
	...
	/* SMTP (Simple Mail Transfer Protocol):
	  ...
	  => 기본적으로 포트 25를 사용하지만, 보안을 강화한 포트 587이나
	  	포트 465(SSL/TLS)가 자주 사용됨
	 
	 * 동작방식
	 
	1. Spring Boot 애플리케이션에서 내가 이메일을 보냄
	2. Google SMTP 서버로 전송 (Google 서버가 "중간 배달자" 역할을 하는 셈)
	3. Google SMTP 서버가 이메일 주소에 맞는 최종 수신자의 이메일 서버
	  (예: Gmail, Yahoo, 회사 이메일 서버)로 이메일을 다시 보냄.
	4. 수신자가 이메일을 받음
	 ...
	 * 전송 방향
	  - HTTP: 클라이언트-서버 간 통신을 기반으로 하며, 클라이언트가 서버에
	  		요청(Request)을 보내고 서버가 응답(Response).
	  - SMTP: 서버 간 통신이 가능하며,
	...
	public JavaMailSender javaMailSender() {
		...
		/* JavaMailSender : 이메일을 보내는데 사용되는 인터페이스,
						JavaMailSenderImpl 클래스를 통해 구현됨 
						SMTP 서버를 사용하여 이메일을 보내기 위한 구성 제공 */
		...
		prop.setProperty("mail.debug", "true");	// 디버그 모드 설정
		
		prop.setProperty("mail.smtp.ssl.trust", "smtp.gmail.com");
		// 신뢰할 수 있는 SMTP 서버 호스트를 지정
		...
		mailSender.setHost("smtp.gmail.com"); // SMTP 서버의 호스트 설정
		mailSender.setPort(587); // SMTP 서버의 포트 587로 설정
		mailSender.setDefaultEncoding("UTF-8"); // 기본 인코딩 설정
		mailSender.setJavaMailProperties(prop);	
		// JavaMail의 속성을 설정(앞에서 정의해 둔 prop에 있는 설정들 적용)
		
		return mailSender;
		// 위 처럼 각종 설정이 적용된 JavaMailSender를 Bean으로 등록하여
		// Spring 어플리케이션에서 이메일을 보내기 위한 구성을 제공
