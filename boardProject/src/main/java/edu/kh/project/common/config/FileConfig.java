...
public class FileConfig implements WebMvcConfigurer{
	/* WebMvcConfigurer : Spring MVC 프레임워크에서 제공하는 인터페이스 중 하나,
						 스프링 구성을 커스터마잊이하고 확장하기 위한 메서드를 제공.
	...
	*/
	...
	// 요청 주소에 따라 서버 컴퓨터의 어떤 경로에 접근할지 설정하는 메서드
	@Override
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		/* ResourceHandlerRegistry : 
		  Spring MVC에서 정적 리소스(이미지, CSS, JS 등)의 요청을 처리하기 위해
		  사용하는 클래스.
		...
		// 서버 폴더 경로 중 C:/uploadFiles/test로 연결하겠다.
		
		registry.addResourceHandler(profileResourceHandler)
		.addResourceLocations(profileResourceLocation);
		// 클라이언트가 /myPage/profile/** 패턴으로 이미지 요청 시
		...
		// => 클라이언트가 /images/board/** 패턴으로 이미지 요청 시
	...
	// => 위에서 만든 MultipartConfigElement를 자동으로 이용함
	@Bean
