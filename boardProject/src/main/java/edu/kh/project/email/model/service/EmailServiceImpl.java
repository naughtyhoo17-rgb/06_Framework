...
public class EmailServiceImpl implements EmailService{
	...
	// SpringTemplateEngine : Thymeleaf를 이용해 HTML 코드 -> java 코드 변환
	
	@Override
	public String sendEmail(String type, String email) {

		// 1. 인증키 생성 및 DB에 저장
		String authKey = creatAuthKey();
		...
		// 2. DB에 저장 성공한 경우, 메일 발송 시도
		MimeMessage mimeMessage = mailSender.createMimeMessage();
		// 메일 발송 시 사용하는 객체 
		
		try {
			
			// 메일 발송을 도와주는 Helper 클래스 (파일 첨부, 템플릿 설정 등을 도움)
			MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true, "UTF-8");
			// - mimeMessage : MimeMessage 객체, 이메일 내용을 담고 있음 (이메일 본문, 제목, 수신자 정보 등)
			// - true : 파일 첨부 여부 지정 ( 파일 첨부 및 내부 이미지 삽입 가능)
			// - "UTF-8" : 이메일 내용이 UTF-8 인코딩으로 전송
			
			// 메일 기본 정보 세팅
			helper.setTo(email); // 받는 사람(수신자)
			helper.setSubject("[boardProject] 회원가입 인증번호입니다."); // 제목
			helper.setText( loadHtml(authKey, type) , true ); // HTML 내용 설정
				// cf) helper.setText("인증번호 입니다 : " + authKey);
				// => 인증번호 입니다 : asdfas
			...
			e.printStackTrace();
			return null; // 메일 발송 실패(예외) 시 null 반환
		}
	...
	private String loadHtml(String authKey, String type) {
		...
		/* Context(org.thymeleaf.context.Context)
		  : 타임리프에서 제공하는 HTML 템플릿에 데이터를 전달하기 위해
		   사용하는 클래스 */
	...
	@Transactional(rollbackFor = Exception.class) // 메서드 레벨이서도 사용 가능
	public boolean storeAuthKey(Map<String, String> map) {
		...
		if(result == 0) { // update가 아닌 처음 저장한 상황, thus insert해줘야 함
			result = mapper.insertAuthKey(map);
		}
		
		// 3. 성공 여부 반환 (true/false 반환)
		return result > 0;
	}
	...
	/* UUID 사용하여 인증키 생성
	  ...
	  매우 낮은 확률로 중복되는 식별자 생성
	  주로 DB 기본키, 고유한 식별자를 생성해야 할 때 사용 */
