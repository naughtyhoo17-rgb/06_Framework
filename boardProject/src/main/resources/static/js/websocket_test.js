...
const testSock = new SockJS("/testSock");
  // - WebSocket 통신을 할 수 있게끔 해주는 객체를 생성함과 동시에
  // 자동으로 http://localhost/testSock으로 연결 요청을 보냄

// 3. 생성된 SockJS 객체를 이용해서 서버에 메시지 전달(by 클라이언트)
const sendMessageFn = (name, str) => {
  ...
  testSock.send(JSON.stringify(obj)); // send() : 서버에 보내는 메서드
}
...
testSock.addEventListener("message", e => {
  ...
  // e.data : 서버로부터 전달받은 message (JSON 형태)
